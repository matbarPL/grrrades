...
pacman::p_load(tidyverse, readxl, ggpubr, riverplot, gridExtra, grid)
options(device = "windows")
setwd("C:\\Users\\Mateusz\\OneDrive\\Dokumenty\\GitHub\\grrrades")
```
...
ggplot(data = grade_per_subject_long %>%
  inner_join(subject_pretty_names,
       ...
       aes(y = value, x = pretty_name, fill = period)) +
  geom_boxplot(show.legend = F) +
  geom_jitter(width = .3) +
  ggtitle("Grades from Mathematics and Portugese") +
  labs(x = "Subject", y = "Grade")
```
```{r}
ggplot(data = grade_per_subject_long %>%
  inner_join(subject_pretty_names,
...
grid.arrange(arrangeGrob(absences.m.plot + theme(legend.position = "none"),
             ...
             top = textGrob("Spare time by the number of absences.", vjust = 1, gp = gpar(fontface = "bold", cex = 1.5)),
...
```{r}
absences_by_romantic_status_m <-
  master_data %>%
  group_by(romantic_m) %>%
  summarise(count = n(), mean_absences = mean(absences_m))
absences_by_romantic_status_to_plot_m <- absences_by_romantic_status_m %>% 
  pivot_longer(cols = count) %>% 
  rename(label = romantic_m, ymax = mean_absences, xmax = value) %>% 
  mutate( ymin = 0) %>% 
  select(-name) %>% 
  select(ymin, xmax, ymax, label) %>% 
  mutate(xmax = cumsum(xmax), xmin = lag(xmax, default = 0)) %>% 
  select(xmin, ymin, xmax, ymax, label)

absences_by_romantic_status_p <-
  master_data %>%
  group_by(romantic_p) %>%
  summarise(count = n(), mean_absences = mean(absences_p))
absences_by_romantic_status_to_plot_p <- absences_by_romantic_status_p %>% 
  pivot_longer(cols = count) %>% 
  rename(label = romantic_p, ymax = mean_absences, xmax = value) %>% 
  mutate( ymin = 0) %>% 
  select(-name) %>% 
  select(ymin, xmax, ymax, label) %>% 
  mutate(xmax = cumsum(xmax), xmin = lag(xmax, default = 0)) %>% 
...
```
```{r}
absences_by_romantic_status_plot_p <- ggplot(absences_by_romantic_status_to_plot_p) +
  geom_rect(aes(xmin = xmin, ymin = ymin, xmax = xmax, ymax = ymax, fill = (xmax - xmin)*(ymax - ymin)),
            color = 'grey78', alpha = .7) +
  theme_minimal() +
  theme(legend.position = 'top') +
  geom_label(aes(y = ymax, label = (xmax - xmin)*(ymax - ymin),
...
absences_by_romantic_status_plot_m <- ggplot(absences_by_romantic_status_to_plot_m) +
  geom_rect(aes(xmin = xmin, ymin = ymin, xmax = xmax, ymax = ymax, fill = (xmax - xmin)*(ymax - ymin)),
            color = 'grey78', alpha = .7) +
  theme_minimal() +
  theme(legend.position = 'top') +
...
```{r}
grid.arrange(arrangeGrob(absences_by_romantic_status_plot_p + theme(legend.position = "none"),
                         absences_by_romantic_status_plot_m + theme(legend.position = "none"),
                         ncol = 2),
             nrow = 2,
             heights = c(10, 1), # relative height of two rows
             top = textGrob("Absences by romantic relationship status.", vjust = 1, gp = gpar(fontface = "bold", cex = 1.5)),
             left = textGrob("Free time after school", rot = 90, vjust = 1),
             bottom = textGrob("Going out with friends", rot = 0, vjust = -2)
)
```
